...
public class ElegantSlideMenuView: UIView {
    ...
    /** 选中按钮时，所要执行的动作 */
    @objc func selectedNameButton(sender: UIButton) {
//        self.viewArray[sender.tag].view.initial()
        ...
        if let cellField_tmp = self.viewArray[sender.tag].view as? TeamRankTab {
            if self.viewArray[sender.tag].isInitial == 0 {
                self.viewArray[sender.tag].isInitial = 1
                cellField_tmp.initial()
            }
        }
    }
